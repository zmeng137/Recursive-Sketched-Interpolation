...
Function_Collection[4] = lambda x: np.exp(-(x - 0.2)**2 / 0.5**2) + 3 * np.cos(x) + 10 * np.sin(10 * x)

# 1D Gaussian functions
w = 0.15
x1 = 0.4
x2 = 0.6
Function_Collection[5] = lambda x: np.exp(-(x-x1)**2/(2*w**2))
Function_Collection[6] = lambda x: np.exp(-(x-x2)**2/(2*w**2))

# High oscillation functions
B_const = 10
Function_Collection[7] = lambda x: np.cos(x * (2 ** B_const)) * np.cos(x * (2 ** B_const) / (4 * np.sqrt(5))) * np.exp(- x * x) + 2 * np.exp(-x)
Function_Collection[8] = lambda x: np.sin(x * (2 ** B_const)) / (2 * np.sqrt(10)) * (np.exp(- x * x * x) + 5 * x * x) - 4 * np.exp(-np.sqrt(x)) + 2.1*x**3
Function_Collection[9] = lambda x: np.cos(x * (2 ** B_const)) * np.exp(- x * x) + 4 * np.exp(x) - 3 * x * x + 10 * x
