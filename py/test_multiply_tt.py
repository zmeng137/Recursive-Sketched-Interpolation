...
def quantics_function_tensor():
    ...
    qtensor_f1, _ = load_quantics_tensor_formula(9, 20)
    qtensor_f2, _ = load_quantics_tensor_formula(10, 20)
...
def load_h5_qtci():
    f1_path = "/home/zmeng5/QTTM/f1_mps.h5"
    
    with h5py.File(f1_path, 'r') as f:
        n = f['num_sites'][()]
        f1_tt = [f[f'tensor_{i+1}'][:] for i in range(n)]
...
#r_max = [2,3,4,5,6,7,8]
#r_max = [5, 10, 15, 20, 25, 30]
r_max = [15,20,25,30,35,40,45,50]
oversampling = [20]
...
for rm in r_max:
    for p in oversampling:
        ...
        TTg_rsi_round = TT_rounding(TTg_rsi, 0, rm-10)
        ...
        if ifEval:
            ...
            error_dict_rsi_round[f"cont_no={2}; rmax={rm-10}; oversampling={p}"] = err_g_rsi_round
